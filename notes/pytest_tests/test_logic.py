...
def test_empty_slug(author_client, form_data):
    ...
    assert Note.objects.count() == 1
    new_note = Note.objects.get()
    excepted_slug = slugify(form_data['title'])
    assert new_note.slug == excepted_slug


def test_author_user_cat_edit_note(author_client, form_data, note):
...
def test_author_user_cat_delete_note(author_client, note_args):
    url = reverse('notes:delete', args=note_args)
    response = author_client.post(url)
    assertRedirects(response, reverse('notes:success'))
    assert Note.objects.count() == 0


def test_other_user_cant_delete_note(not_author_client, note_args):
    url = reverse('notes:delete', args=note_args)
